...
public class SudokuApp extends JFrame {
   ...
   public SudokuApp(String difName, int difficulty) {
      ...
      // restart.addActionListener(dispose());
      // menu.add(restart);
      ...
      // banyak yang di masking
      for (int i = 0; i < difficulty*9; i++) { //sampai difficulty
   ...
   private class InputListener implements ActionListener
   {
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         //mengecek apakah angka pada row sudah benar dan merubah warna background jika salah atau benar
         ...
         for (int row = 0; row < GRID_SIZE; ++row) {
            for (int col = 0; col < GRID_SIZE; ++col) {
               if (toCheckNumber == puzzle[rowSelected][colSelected]) {
                  // tfCells[rowSelected][colSelected].setText("True");
